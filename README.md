...
| Grain | Balancing authority × hour |
...
3. **Model** (`src/03_build_warehouse.sql`) — a DuckDB star schema: two fact
...
`powerbi/grid_stress.pbip` — six pages over a star schema with 33 DAX measures,
all relationships many-to-one in a single filter direction.
...
```bash
git clone https://github.com/josealemanm/grid-stress-dashboard.git
